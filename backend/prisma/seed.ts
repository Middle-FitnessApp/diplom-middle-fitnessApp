...
async function main() {
	...
	for (const client of clients) {
		...
		// ГАРАНТИРОВАННО выбираем одного тренера как ACCEPTED для каждого клиента
		const acceptedTrainer = faker.helpers.arrayElement(invitedTrainers)
		...
		for (const trainer of invitedTrainers) {
			...
			if (trainer.id === acceptedTrainer?.id) {
				...
				// Остальные приглашения: случайно PENDING или REJECTED
				status = faker.datatype.boolean(0.3) ? 'REJECTED' : 'PENDING'
			}
	...
	// Создаём 200-300 комментариев к прогрессам от тренеров
	const progresses = await prisma.progress.findMany()
	const numComments = faker.number.int({ min: 200, max: 300 })
	...
	for (const relation of acceptedRelations) {
		...
		for (let i = 0; i < numMessages; i++) {
			...
			const messageText = faker.helpers.arrayElement([
				...
				'Спасибо за поддержке! Мотивация на высоком уровне 🔥',

...
- [Отмена плана питания клиента](#отмена-плана-питания-клиента)
- [Редактирование плана питания клиента](#редактирование-плана-питания-клиента)
...
Тренер может отменить активный план питания клиента.

**Endpoint:** `DELETE /api/trainer/clients/:clientId/nutrition/:planId`
...
- **401 Unauthorized** - Отсутствует или недействителен access token
- **403 Forbidden** - Тренер не работает с этим клиентом (статус не `ACCEPTED`)
- **404 Not Found** - Активный план питания не найден
...
1. Связь тренер-клиент со статусом `ACCEPTED`
2. План принадлежит указанному клиенту
3. План активен (`isActive: true`)

**Действие:**

- План деактивируется (`isActive: false`)
- План переходит в историю клиента
- Клиент больше не видит этот план как активный

---

## Редактирование плана питания клиента

Тренер может изменить активный план питания клиента: список дней или дату начала.

**Endpoint:** `PUT /api/trainer/clients/:clientId/nutrition/:planId`
...
| --------- | ------ | ------------ | ------------------------------ |
| dayIds    | array  | Нет          | Массив ID дней для назначения  |
| startDate | string | Нет          | Дата начала плана (ISO формат) |

**Примечание:** Необходимо указать хотя бы одно поле для обновления.
...
```json
{
	"dayIds": ["cm4day001", "cm4day003", "cm4day005"]
}
```
...
```json
{
	...
	"plan": {
		...
		"subcategory": {
			"id": "cm4subcat001",
			"name": "Для мужчин 70-80кг",
			"description": "План для набора мышечной массы"
		},
		"assignedDays": ["cm4day001", "cm4day003", "cm4day005"],
		"startDate": "2025-12-10T00:00:00.000Z",
...
- **400 Bad Request**
  ...
  - Указаны несуществующие ID дней
...
2. План принадлежит указанному клиенту
3. План активен (`isActive: true`)
4. Если указаны `dayIds` - все дни существуют в подкатегории плана

**Особенности:**

- Можно изменить только один параметр или оба сразу
- Изменение `dayIds` на пустой массив `[]` означает назначение всех дней подкатегории
- Изменение `startDate` влияет на расчёт текущего дня в цикле
- Поле `updatedAt` обновляется автоматически

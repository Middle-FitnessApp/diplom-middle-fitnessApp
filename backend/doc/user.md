...
3. **Проверка дубликатов:**
   ...
   - Если тренер ранее отклонил приглашение (статус `REJECTED`) → повторная отправка (статус меняется на `PENDING`)
...
- **400 Bad Request** - Приглашение этому тренеру уже отправлено (статус PENDING)
...
- `message` - подтверждающее сообщение с именем тренера
- `deactivatedNutritionPlans` - количество деактивированных планов питания
...
3. **Удаление данных (транзакция):**
   ...
   - Деактивируются все найденные планы питания (isActive = false)
...
- Деактивируются **только планы питания**, назначенные этим тренером
- **Комментарии к отчетам остаются** - это важная история взаимодействия
- После отмены клиент может работать с другим тренером
- Операция **необратима** для связи, но планы можно реактивировать при новом сотрудничестве

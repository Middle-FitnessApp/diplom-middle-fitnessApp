...
Проект использует [Vitest](https://vitest.dev/) для unit-тестирования. Все тесты находятся в папке `backend/tests/`.
...
```bash
npm test -- create-progress.test.ts
```
...
После завершения откройте файл `coverage/index.html` в браузере. Папка `coverage/` содержит полный HTML-отчёт о покрытии кода тестами.
Что показывает отчёт: общий процент покрытия по проекту и по каждому файлу; цветовую индикацию строк кода; детальную статистику по ветвлениям (branches) и функциям.

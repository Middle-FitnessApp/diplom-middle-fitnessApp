...
export const useThemeClasses = () => {
	...
	return {
		...
		borderWhite: isDark ? 'border-slate-600' : 'border-gray-700',
		...
		hoverBg: isDark ? 'hover:bg-slate-700' : 'hover:bg-sky-700',
		...
		// Уведомления
		notificationUnreadBg: isDark ? 'bg-blue-900/20' : 'bg-blue-50',
		notificationUnreadBorder: isDark ? 'border-blue-700' : 'border-blue-200',
		notificationReadBg: isDark ? 'bg-slate-800' : 'bg-white',
		notificationReadBorder: isDark ? 'border-slate-700' : 'border-gray-200',
		notificationAvatarBg: isDark ? 'bg-slate-700' : 'bg-gray-100',

...
interface NutritionCategoryCardProps {
	...
	handleCategoryClick: (category: NutritionCategory) => void
}
...
export const NutritionCategoryCard = ({
	...
	const getCategoryIcon = (category: NutritionCategory) => {
		return openedCategoryId === category.id ? <DownOutlined /> : <RightOutlined />
	}
	...
	return (
		<Card
			hoverable
			className={`border-muted hover:shadow-lg transition-all duration-300 background-light ${
				openedCategoryId === category.id ? 'ring-2 ring-primary' : ''
			}`}
